...
def load_ply_data(path, max_sh_degree=3):
    ...
    if "f_dc_0" in plydata.elements[0]:
        ...
        """
        #features_all = torch.cat((torch.tensor(features_dc, device="cpu"), torch.tensor(features_extra, device="cpu")), 2)
        features_all = torch.tensor(features_dc, device="cpu")
        
        colours = computeColorFromLowDegSH(features_all)
    
    elif "red" in plydata.elements[0]:

        colours = torch.zeros((xyz.shape[0], 3), device="cpu", dtype=torch.double)
        colours[:, 0] = torch.tensor(plydata.elements[0]["red"], device="cpu", dtype=torch.double)
        colours[:, 1] = torch.tensor(plydata.elements[0]["green"], device="cpu", dtype=torch.double)
        colours[:, 2] = torch.tensor(plydata.elements[0]["blue"], device="cpu", dtype=torch.double)
    ...
    opacities = (1 / (1 + torch.exp(torch.tensor(-opacities, device="cpu")))).type(torch.float).squeeze(1) # torch.full(opacities.shape, 0.05, device="cpu")-

    xyz_tensor = torch.tensor(xyz, device="cpu")
    scales_tensor = torch.tensor(scales, device="cpu")
    rots_tensor = torch.tensor(rots/np.expand_dims(np.linalg.norm(rots, axis=1), 1), device="cpu") 
...
def load_splat_data(path):
    ...
    xyz_tensor = torch.tensor(xyz, device="cpu")
    scales_tensor = torch.tensor(np.log(scales), device="cpu")
    colours_tensor = torch.tensor(colours[:, :3] / 255, device="cpu")
    opacities = torch.tensor(colours[:, 3] / 255, device="cpu")
    
    rots_tensor = torch.tensor((rots.astype(np.float32) - 128) / 128, device="cpu")
