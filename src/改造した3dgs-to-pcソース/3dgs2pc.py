...
def create_new_gaussian_points(num_points_to_sample, means, covariances, colours, mahalanobis_distance_std=2, num_attempts=5, normals=None, max_num_gen_attemps=3, device="cpu"):
    """
...
def generate_pointcloud(gaussians, num_points, contributions=None, mahalanobis_distance_std=2, exact_num_points=False, calculate_normals=True, 
                                               num_sample_attempts=5, device="cpu", quiet=False):
...
def main():
    ...
    quiet = False
    OUTPUT_FOLDER = '3DGS2PC'
    
    argv = sys.argv
    argc = len(argv)

    print('%s converts 3DGS ply to RGB ply' % argv[0])
    print('[usage] python %s <input folder (containing 3DGS PLYs)> [<output folder>]' % argv[0])

    if argc < 2:
        quit()

    paths = glob.glob(os.path.join(argv[1], '*.ply'))
    ...
    for i, path in enumerate(paths):
        ...
        total_point_cloud, surface_point_cloud = convert_3dgs_to_pc(path, None, None, pointcloud_settings)
        ...
        # Save point cloud
        save_xyz_to_ply(total_point_cloud.points, output_path, rgb_colors=total_point_cloud.colours, 
                                                                    normals_points=total_point_cloud.normals, chunk_size=10**6, quiet=quiet)
        ...
        if False:
            ...
            generate_mesh(surface_point_cloud.points, surface_point_cloud.colours, surface_point_cloud.normals, mesh_output_path, 
                          depth = 10, laplacian_iters = 10)
