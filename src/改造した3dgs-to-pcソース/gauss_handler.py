...
class Gaussians():
    ...
    def set_default_filter(self):
        self.filter_indices = torch.full((self.xyz.shape[0],), True, dtype=torch.bool, device=torch.device(f"cpu")) 
    ...
    def calculate_normals(self):
        ...
        normal_matrices = torch.zeros(self.xyz.shape, dtype=torch.float, device="cpu")
    ...
    def regularise_covariances(self, covariances, mask=None, epsilon=5e-7):
        ...
        eye_matrix = epsilon * torch.eye(3, device="cpu").expand(mask.sum(), 3, 3)
    ...
    """def apply_k_nearest_neighbours(self, k=10):
        ...
        invalid_gaussians = torch.tensor(total_distances > max_dist, device="cpu")"""
